...
SELECT pool, time_bucket('5 min', time) AS time,
    ...
    COUNT(CASE WHEN asset_amount < 0 AND event_type = 'swap' THEN 1 ELSE NULL END) AS buy_count,
    ...
    COUNT(CASE WHEN rune_amount < 0 AND event_type = 'swap' THEN 1 ELSE NULL END) AS sell_count,
    SUM(CASE WHEN rune_amount < 0 AND event_type = 'swap' THEN -rune_amount ELSE 0 END) AS sell_volume,
    SUM(units) AS units_changes,
    COUNT(CASE WHEN event_type = 'stake' THEN 1 ELSE NULL END) AS stake_count,
    COUNT(CASE WHEN event_type = 'unstake' THEN 1 ELSE NULL END) / 3 AS withdraw_count
FROM pools_history
