...
func (s *TimeScaleSuite) TestAssetAdded(c *C) {
	...
	// Zero pool depth
	depth, err := s.Store.GetPoolDepth(assetBolt)
	c.Assert(err, IsNil)
	c.Assert(depth, Equals, uint64(0))
	depth, err = s.Store.GetPoolAssetDepth(assetBolt)
	c.Assert(err, IsNil)
	c.Assert(depth, Equals, uint64(0))
	...
	c.Assert(err, IsNil)
	c.Assert(assetAdded, Equals, int64(1000))
	depth, err = s.Store.GetPoolDepth(assetToml)
	c.Assert(err, IsNil)
	c.Assert(depth, Equals, uint64(0))
	depth, err = s.Store.GetPoolAssetDepth(assetToml)
	c.Assert(err, IsNil)
	c.Assert(depth, Equals, uint64(1000))
}
